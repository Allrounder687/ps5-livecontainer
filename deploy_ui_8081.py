...
HOST = "192.168.0.208"
ELFLDR_PORT = 9021
HTTP_PORT = 8081  # Port 8081 for the visual Web UI Dashboard
...
<body>
    ...
    <div class="upload-section">
        <p style="font-weight: 600; font-size: 1.05rem; margin-bottom: 6px;">🎮 LiveContainer Host Engine Running on PS5</p>
...
def build_ui_elf(out_path="livecontainer_ui.elf"):
    ...
    # sockaddr_in for port 8081 (0x1F91)
    sockaddr_in = bytearray(16)
    sockaddr_in[0] = 16 # sin_len
    sockaddr_in[1] = 2  # AF_INET
    sockaddr_in[2] = 0x1F # port high byte (8081 = 0x1F91)
    sockaddr_in[3] = 0x91 # port low byte
    ...
    struct.pack_into('<Q', buf, 24, 0x100)      # entry: 0x100
    struct.pack_into('<Q', buf, 32, 64)         # phoff: 64
    struct.pack_into('<Q', buf, 40, sh_off)     # shoff: 0x1F80
...
def deploy():
    print("========================================================")
    print(f" Deploying Visual Web Dashboard to PS5 (Port {HTTP_PORT})")
    print("========================================================")

    sz = build_ui_elf("livecontainer_ui.elf")
    print(f"[1] Built livecontainer_ui.elf ({sz} bytes)")

    with open("livecontainer_ui.elf", "rb") as f:
        data = f.read()
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=5) as resp:
            ...
            print("\n========================================================")
            print(" [SUCCESS] HTML WEB DASHBOARD IS LIVE ON PORT 8081!")
            print("========================================================")
            print(f"Content-Type: {resp.headers.get('Content-Type')}")
            print(f"HTML Preview (first 250 chars):\n{content[:250]}...")
            print("========================================================")
            return True
